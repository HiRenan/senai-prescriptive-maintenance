...
| --- | --- | --- | --- |
| `uv run --frozen poe check` | Gate agregado antes da demo. | Formatação somente leitura, lint, tipagem, suíte Pytest, testes Node e regressão local do frontend AWS. | Não inclui o teste Chromium nem prova AWS live ou qualidade semântica. |
...
| `uv run --frozen poe aws-demo-frontend-regression` | Gate offline de publicação da SEN-75. | Staging fechado, inventário estático, runtime config, headers, publicação/invalidação e smoke do frontend com fakes locais. | Não consulta S3, CloudFront, Cognito ou API Gateway reais. |
