...
| `apps/api/src/prescriptive_maintenance/persistence/` | Metadados imutáveis de análise/documento/versão/chunk/evidência, `index_id` e vizinhos opacos ordenados, registry de ciclo/auditoria, UoW, adapter psycopg, CAS transacional e migrações reversíveis para metadados e índice de similaridade. | As rotas usam somente metadados; conteúdo, features, labels, vetores e narrativas não são persistidos. Migrações continuam explícitas. |
...
| `apps/api/src/prescriptive_maintenance/modeling/` | Busca k-NN v3 em memória sobre 18 features, `StandardScaler` de treino, distância euclidiana, condição candidata baseada em históricos, política fechada dos cinco estados operacionais, suporte heurístico, abstenção tipada, gate open-set de promoção somente em validação, adapter `ModelPort`, artefato NumPy/JSON íntegro, índice derivado versionado com adapters exatos em memória e PostgreSQL/pgvector e harness temporal com métricas candidatas, seletivas, open-set e de ranking. | Só é ligada às rotas por `artifacts`, não calibra probabilidade, não usa o teste no fit ou no gate, não faz tuning pelo holdout, não usa busca aproximada ou GPU; o gate real é inviável, o ranking ainda usa linhas e a avaliação pós-hoc não aprova o modelo para operação. |
