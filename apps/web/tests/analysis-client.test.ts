...
import { createAnalysisClient } from "../src/api/analysis-client";
...
function respondWith(status: number, body: unknown, options: { raw?: string } = {}) {
  return async () =>
...
function client(fetchImpl: (input: any, init: any) => Promise<any>) {
...
test("uma análise bem-sucedida devolve o resultado do contrato", async () => {
  ...
  assert.deepEqual((output as any).response, example);
});
...
test("o 422 preserva os campos recusados pelo contrato", async () => {
  ...
  const output: any = await client(respondWith(422, envelope)).requestAnalysis(
...
test("401 e 403 são classificados como autenticação sem resultado", async () => {
  for (const status of [401, 403]) {
    const output: any = await client(respondWith(status, {})).requestAnalysis(
      requestExample("normal"),
    );
    assert.equal(output.ok, false);
...
test("um status fora do contrato é reportado como inesperado", async () => {
  const output: any = await client(respondWith(418, {})).requestAnalysis(
    requestExample("normal"),
  );
  assert.equal(output.ok, false);
...
test("uma falha de rede não é confundida com resposta da API", async () => {
  const output: any = await client(async () => {
...
test("uma implementação que ignora abort não aplica a resposta tardia", async () => {
  ...
  const output: any = await lateClient.requestAnalysis(requestExample("normal"));
...
test("um corpo 200 incompleto é recusado antes de chegar ao laudo", async () => {
  for (const missing of ["analysis_id", "model_id", "support", "neighbors", "warnings"]) {
    ...
    const output: any = await client(respondWith(200, broken)).requestAnalysis(
      requestExample("normal"),
    );
 ...
 */
async function kindFor(outcome: string, mutate: (body: any) => void): Promise<string> {
...
async function assertAllMalformed(
  cases: readonly [string, string, (body: any) => void][],
) {
...
test("somente o status de sucesso do contrato vira resultado", async () => {
  ...
  for (const status of [201, 202, 206]) {
    const output: any = await client(respondWith(status, example)).requestAnalysis(
      requestExample("normal"),
    );
  ...
  const empty: any = await client(async () => new Response(null, { status: 204 }))
    .requestAnalysis(requestExample("normal"));
...
test("um corpo que nunca termina cai no tempo limite, não em laudo", async () => {
  ...
  const output: any = await client(fetchImpl).requestAnalysis(requestExample("normal"));
  assert.equal(output.ok, false);
  assert.equal(output.failure.kind, "timeout");
});
...
test("o contrato gerado publica o padrão de cada família de identificador", () => {
  ...
  for (const [schema, field, prefix] of declared) {
    const node: any = RESPONSE_SCHEMAS[schema].properties[field];
