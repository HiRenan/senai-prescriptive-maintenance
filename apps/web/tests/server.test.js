...
test("a raiz serve o documento do painel com cabeçalhos de segurança", async () => {
  ...
  assert.match(body, /Demonstração operacional/);
  assert.match(body, /Gestão documental/);
});
...
test("os módulos e o estilo do painel são servidos com o tipo correto", async () => {
  ...
  const documentContract = await fetch(`${origin}/generated/document-contract.js`);
  assert.equal(documentContract.status, 200);
});
