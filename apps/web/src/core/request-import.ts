...
export function checkImportSize(
  bytes: number,
): { ok: false; issues: readonly ValidationIssue[] } | null {
 ...
 */
export function importAnalysisRequest(
  text: string,
  ...
  for (const key of Object.keys(rawFeatures)) {
    if (!known.has(key as keyof AnalysisFeatures)) {
      issues.push({
        field: key,
  ...
  if (rawTopK !== undefined) {
    if (!Number.isInteger(rawTopK)) {
      ...
      const candidate = rawTopK as number;
