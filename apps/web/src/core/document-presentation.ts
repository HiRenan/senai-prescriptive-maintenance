...
import { documentFieldLabel } from "./document-registration";
import type { DocumentFailure } from "../api/document-client";
...
export interface DocumentView {
  ...
  tone: string;
  statement: string;
  metadata: readonly { label: string; value: string; note: string | null }[];
  updatedAt: string;
  currency: {
    state: string;
    label: string;
  ...
  decision: { label: string; text: string } | null;
  failure: { code: string; message: string } | null;
  actions: readonly DocumentAction[];
  actionsNote: string | null;
}

export interface DocumentFailureView {
  title: string;
  statement: string;
  nextStep: string;
  status: number | null;
 ...
 */
export function documentActions(status: string): readonly DocumentAction[] {
  return Object.freeze(
    ACTIONS.filter((action) => DOCUMENT_ACTION_POLICY[action.name].includes(status)),
  );
}

function resolveCurrency(document: DocumentResponse): {
  state: string;
  label: string;
  explanation: string;
  supersededBy: string | null;
  ...
  return {
    state,
    ...CURRENCY_COPY[state as "current" | "not_current"],
    supersededBy: null,
  };
}
 ...
 */
export function presentDocument(document: DocumentResponse): DocumentView {
 ...
 */
export function presentDocumentList(documents: readonly DocumentResponse[]): {
  empty: boolean;
  emptyMessage: string;
  total: number;
 ...
 */
export function presentDocumentFailure(failure: DocumentFailure): DocumentFailureView {
 ...
 */
export function describeRegistration(current: DocumentResponse): string {
