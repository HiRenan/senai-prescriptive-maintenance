...
function isDocumentId(documentId) {
  const match = documentIdExpression.exec(documentId);
  return match !== null && match[0] === documentId;
}

/**
 * @param {import("../generated/document-contract.js").DocumentOperation} operation
 * @param {string} pathname
 * @returns {boolean}
 */
function matchesDocumentOperation(operation, pathname) {
 ...
 * @returns {boolean}
 */
export function isAllowedApiRequest(method, pathname) {
 ...
 * @param {{ getAccessToken: () => string | null, clear: () => void }} options.session
 * @param {typeof fetch} [options.fetchImpl]
 * @param {() => void} [options.onAuthenticationRequired]
